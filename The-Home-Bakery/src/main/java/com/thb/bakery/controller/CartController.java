...
public class CartController {
    ...
    // ========================================================================
    // ADD TO CART – NOW SUPPORTS ADDON QUANTITY
    // ========================================================================
    ...
    public ResponseEntity<Map<String, Object>> addToCart(@RequestBody Map<String, Object> request) {
        ...
        try {
            ...
            // Parse addons with quantity
            List<Map<String, Object>> addonList = request.get("addonIds") instanceof List ?
                    (List<Map<String, Object>>) request.get("addonIds") : new ArrayList<>();

            Map<Addon, Integer> addonMap = new HashMap<>();
            for (Map<String, Object> entry : addonList) {
                Long addonId = Long.valueOf(entry.get("id").toString());
                int qty = entry.containsKey("quantity") ?
                        Integer.parseInt(entry.get("quantity").toString()) : 1;

                Addon addon = addonRepository.findById(addonId).orElse(null);
                if (addon != null) {
                    addonMap.put(addon, Math.max(1, qty));
                }
            }
            ...
            if (itemType == CartItemEntity.ItemType.PRODUCT) {
                ...
                if (existing.isPresent()) {
                    ...
                    item.getAddonQuantityMap().clear();
                    item.getAddonQuantityMap().putAll(addonMap);
                } else {
                ...
                Long snackId = request.containsKey("snackId") ?
                        Long.parseLong(request.get("snackId").toString()) : null;
    ...
    // ========================================================================
    // GET CART ITEMS – NOW INCLUDES addon.quantity
    // ========================================================================
    ...
    public ResponseEntity<List<Map<String, Object>>> getCart(@RequestParam Long userId) {
        ...
        try {
            ...
            for (CartItemEntity item : items) {
                ...
                List<Map<String, Object>> addonList = item.getAddonQuantityMap().entrySet().stream()
                        .map(entry -> {
                            Addon a = entry.getKey();
                            Integer qty = entry.getValue();
                            Map<String, Object> m = new HashMap<>();
                            m.put("id", a.getId());
                            m.put("itemKey", a.getItemKey());
    ...
    public ResponseEntity<Map<String, Object>> updateCartItem(@RequestBody Map<String, Object> request) {
        ...
        try {
            ...
            if (opt.isEmpty() || !opt.get().getUserId().equals(userId)) {
                return ResponseEntity.badRequest().body(Map.of("error", "Item not found or unauthorized"));
            }

            CartItemEntity item = opt.get();

            // Update product quantity
            if (request.containsKey("quantity")) {
                int qty = Integer.parseInt(request.get("quantity").toString());
                if (qty <= 0) return ResponseEntity.badRequest().body(Map.of("error", "Quantity must be positive"));
                item.setQuantity(qty);
            }

            // Update addons with their quantities
            if (request.containsKey("addons")) {
                List<Map<String, Object>> addonsUpdate = (List<Map<String, Object>>) request.get("addons");
                item.getAddonQuantityMap().clear();
                for (Map<String, Object> a : addonsUpdate) {
                    Long addonId = Long.parseLong(a.get("id").toString());
                    int qty = Math.max(1, Integer.parseInt(a.get("quantity").toString()));
                    Addon addon = addonRepository.findById(addonId).orElse(null);
                    if (addon != null) {
                        item.addAddonWithQuantity(addon, qty);
                    }
                }
            }
    ...
    // ========================================================================
    // MERGE, CLEAR, ETC (UNCHANGED – just kept for completeness)
    // ========================================================================
